...
//seller id getter
$seller_getter = mysqli_query($conn, "SELECT ID,FirstName,LastName FROM users WHERE EmailID='$seller_email'");
/*    if($seller_getter){
        echo "select";
    }else{
...
if (isset($_GET['ID'])) {
    $noteid_allow = $_GET['ID'];
    $downloader_allow = $_GET['DownloaderID'];
    $allow_download_enable = mysqli_query($conn, "UPDATE downloads SET IsSellerHasAllowedDownload=1,AttachmentDownloadedDate=NOW() 
                             WHERE NoteID=$noteid_allow AND DownloaderID=$downloader_allow");

    //buyer getter
    $buyer_name_getter = mysqli_query($conn, "SELECT FirstName,LastName,EmailID FROM users WHERE ID=$downloader_allow");
...
if (isset($_POST['search'])) {
    ...
    $query = "SELECT DISTINCT downloads.NoteID,downloads.DownloaderID,downloads.NoteTitle,downloads.NoteCategory,users.EmailID,
            users_details.Phone_No,downloads.IsPaid,downloads.PurchasedPrice,downloads.AttachmentDownloadedDate,country.Country_Code FROM downloads 
    ...
    $result_num = mysqli_query($conn, "SELECT COUNT(downloads.ID) FROM downloads LEFT JOIN referencedata ON downloads.IsPaid=referencedata.ID 
                            LEFT JOIN users ON downloads.DownloaderID=users.ID 
                            LEFT JOIN users_details ON users_details.UserID=downloads.DownloaderID
                            LEFT JOIN country ON users_details.Country=country.ID 
                            WHERE downloads.NoteTitle LIKE '%$search_result%' OR downloads.NoteCategory LIKE '%$search_result%' OR users.EmailID LIKE 
                            '%$search_result%'  OR users_details.Phone_No LIKE '%$search_result%' OR referencedata.Value LIKE '%$search_result%' OR downloads.PurchasedPrice LIKE '%$search_result%'
                            AND downloads.IsPaid=4
                            SellerID= $seller_id  AND IsSellerHasAllowedDownload=0 LIMIT $start_from,$limit");	
    ...
    $query = "SELECT DISTINCT downloads.NoteID,downloads.DownloaderID,downloads.NoteTitle,downloads.NoteCategory,users.EmailID,
    users_details.Phone_No,downloads.IsPaid,downloads.PurchasedPrice,downloads.AttachmentDownloadedDate,country.Country_Code FROM downloads 
    LEFT JOIN referencedata ON downloads.IsPaid=referencedata.Value 
    LEFT JOIN users ON downloads.DownloaderID=users.ID 
    LEFT JOIN users_details ON users_details.UserID=downloads.DownloaderID
    LEFT JOIN country ON users_details.Country=country.ID 
...
$result_num = mysqli_query($conn, "SELECT COUNT(downloads.ID) FROM downloads LEFT JOIN referencedata ON downloads.IsPaid=referencedata.Value 
                    LEFT JOIN users ON downloads.DownloaderID=users.ID 
                    LEFT JOIN users_details ON users_details.UserID=downloads.DownloaderID
                    LEFT JOIN country ON users_details.Country=country.ID 
