...
//update note status when admin publish note 
if(isset($_POST['inreview_btn'])){
  $inreview_id=$_POST['inreview_note_id'];
  $inreview_query=mysqli_query($conn,"UPDATE notes SET Status=8,Actioned_By=$userid WHERE ID=$inreview_id");

}
if(isset($_POST['approve_btn'])){
  $publish_id=$_POST['approve_note_id'];
  $publish_query=mysqli_query($conn,"UPDATE notes SET Status=9,Actioned_By=$userid,PublishedDate=NOW() WHERE ID=$publish_id");
   
}

if(isset($_POST['reject_yes'])){
    $reject_id = $_POST['reject_note_id'];
    $remark = $_POST['remark'];

    echo $userid;
    echo $reject_id;
    $reject_query = mysqli_query($conn ,"UPDATE notes SET Status=10,Actioned_By=$userid,Admin_Remarks='$remark' 
                WHERE ID=$reject_id " );
                        ...
                        <li class="logged-in-user-photo-li">
                            ...
                            <?php
                                if (isset($_SESSION['email'])) {
                                    ...
                                    $exist_userid_in_profile_checker = mysqli_query($conn, "SELECT 1 FROM users_details WHERE UserID=$user_id");
                                    $userid_count = mysqli_num_rows($exist_userid_in_profile_checker);

                                    if ($userid_count != 0) {
                                        $dp_path_getter = mysqli_query($conn, "SELECT Profile_Pic FROM users_details WHERE UserID=$user_id");
            ...
            <div class="container">
                <div class="row no-gutters all-notes">
                    <div class="col-lg-6 col-md-6 col-sm-12 col-12 text-left">
                        <p class="note-heading">Note Under Review</p>
                    </div>
                </div>
            </div>
        </div>
 ...
 <!-- Dashboard content End-->
    ...
    <script type="text/javascript">
          ...
          function filterData(){
            ...
            $.ajax({
              url:"notes-under-review-ajax.php",
              type:"GET",
              data:{
                  search:search_txt,
